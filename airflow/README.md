...
4. Update the schema: The template includes a single document class called `Docs`. 
...
This is for simplicity of the reference architecture and, even with a simple schema, Ask Astro performs very well. When building a new usecase from the reference architecture it will likely be necessary to optimize the schema for the specific document types and use-case.
...
- Set the `seed_baseline_url` variable to 'https://astronomer-demos-public-readonly.s3.us-west-2.amazonaws.com/ask-astro/baseline_data_v2.parquet'
...
- Edit streamlit app with WEAVIATE_CLASS and _WEAVIATE_CONN_ID
